...
static ssize_t mxt_glove_enable_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	char c;

	c = data->glove_enabled ? '1' : '0';
	return scnprintf(buf, PAGE_SIZE, "%c\n", c);
}
...
static ssize_t mxt_glove_enable_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	u8 i;
	...
	if (kstrtou8(buf, 0, &i) == 0 && i < 2) {
		data->glove_enabled = (i == 1);

		dev_dbg(dev, "%s\n", i ? "glove enabled" : "glove disabled");
		ret = count;
	} else {
		dev_dbg(dev, "glove write error\n");
		ret = -EINVAL;
	}
...
static DEVICE_ATTR(glove_enable, S_IWUSR | S_IRUSR, mxt_glove_enable_show,
		   mxt_glove_enable_store);
