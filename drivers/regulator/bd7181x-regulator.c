					...
					int min_uA, int max_uA)
{
	...
	for (i = 0; i < ARRAY_SIZE(bd7181x_wled_currents); i++) { // SET TO MIN VALUE
		...
		{
			printk(KERN_INFO "REGULATOR bd7181x_led_set_current_limit i :%d\n",i);
...
static struct bd7181x_regulator bd7181x_regulators[] = {
	...
	BD7181X_BUCK_REG(BUCK5, BD7181X_REG_BUCK5_VOLT, BD7181X_REG_BUCK5_MODE,  1800000, 3000000, 50000),
...
static DEVICE_ATTR(out32k_mode, 0444, show_mode, set_mode);
static DEVICE_ATTR(out32k_value, 0444, show_value, set_value);
